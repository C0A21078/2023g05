...
<body>
  ...
  <div style="display: flex; align-items: center;">
    ...
    <span style="font-size: 45px; font-weight: bold;">ハリーポッター 魔法の覚醒</span>
  </div>
  ...
  <div class="popup-overlay" id="popup-overlay">
    <div class="popup-content">
      ...
      <div id="collapse-content">
        <p>
          魔法を極めよ
          魔法ワールドRPG『ハリー・ポッター：魔法の覚醒』6月27日(火) 日本リリース！
        </p>
        <p>
          ▼白熱のカードバトルーー呪文を放て、魔法動物を召喚せよ
          "エクスペリアームス！" "ステューピファイ!" 決闘クラブで白熱するリアルタイムカードバトル。
          強力な呪文や魔法生物を繰り出すことができるカードと懐かしいあの人物たちの力を借りることができる共鳴で、
          最強の魔法使いになれるデッキを構築しよう。知略や戦術を駆使して呪文を唱えたりかわしたり。勝利はあなたの指先の中にある！
        </p>
        <p>
          ▼映画のような冒険
          物語の舞台は闇の帝王を打倒してから数年が経ったホグワーツ。平和が訪れたかのように思えたが……。
          個性豊かな新入生たちと原作おなじみの登場人物たちと魔法の世界の謎に挑む！
        </p>
        <p>
          ▼魔法ワールドを探検
          トゥーン調のファンシーなアートが特徴の「魔法の覚醒」の世界の中で、魔法のような時間を。ダイアゴン横丁を散策して魔法のお店に驚いたり、
          ホウキでホグワーツを飛び回ったり、禁じられた森の奥深くでユニコーンやケンタウロスの謎を学んだり……。あなたの夢はすべてここで叶います！
        </p>
        <p>
          ◆高品質なアートと音楽
        ...
        <p>
          ▼魅力的な授業
          フリットウィック先生と浮遊の魔法を学び、スラグホーン先生と魔法薬を調合したり、トレローニ先生と水晶玉を覗いたり……。クラスのみんなとも協力して、
          魔法界の偉大な人物を目指し、あらゆる困難を乗り越えよう！
        </p>
        <p>
          ▼魔法生活を楽しもう
          クィディッチでゴールを決めたり、舞踏会で勇気を出して大切な人をダンスに誘ったり、友達とサークルを作ったり……。
          広がる魔法ワールドで、あなただけの魔法ライフを送ろう。
        </p>
      </div>
    </div>
  </div>
...
<%
  try {
    ...
    // ステートメントの作成とSQL文の実行
    String sql = "SELECT star_rating, COUNT(*) as count FROM app_haripota_review GROUP BY star_rating";
...
<%
  try {
    ...
    // ステートメントの作成とSQL文の実行
    String sql = "SELECT * FROM app_haripota_review";
    Statement stmt = con.createStatement();
    ResultSet rs = stmt.executeQuery(sql);
