...
<body>
  ...
  <div style="display: flex; align-items: center;">
    ...
    <span style="font-size: 45px; font-weight: bold;">フィッシュダム(Fishdom)</span>
  </div>
  ...
  <div class="popup-overlay" id="popup-overlay">
    <div class="popup-content">
      ...
      <div id="collapse-content">
        <p>
          フィッシュダムは初めて？大きく息を吸って、水中でのマッチ3の世界に飛び込みましょう！
        </p>
        <p>
          楽しくてハマるマッチ3に挑戦して、かわいい魚が暮らすアクアリウムをデコレーションしましょう。
          魚にエサをあげたり遊んだり、魚同士の会話も見逃せない！魚がいっぱいのアクアリウムを今すぐ見てみましょう！
        </p>
        <p>
          注目ポイント：
          ● 他にはないゲーム：入れ替えてマッチを作り、デザインして飾って、魚と遊んで世話をするおもしろさを1つのゲームで実現！
          ● 楽しくて思わずハマるマッチ3レベルがいっぱい
          ● 個性的で楽しい3Dの魚と一緒にワクワクするアクアリウムを探索！
          ● 各種の装飾アイテムでアクアリムをデコレーション
          ● スキューバマスクをつけて美しいグラフィックを堪能
          ● 一緒にダイビング：Facebookの友達とプレイ！
        </p>
        <p>
          ◆高品質なアートと音楽
        ...
        <p>
          ご注意ください！フィッシュダムは無料ゲームですが、実際の通貨で購入可能なゲーム内アイテムもあります。
          アプリ内課金は、ご利用デバイスの機能制限設定からオフにできます。
        </p>
        <p>
          フィッシュダムはいかがですか？詳しい情報はこちら：
          Facebook: facebook.com/Fishdom
          Instagram: instagram.com/fishdom_mobile/
          Twitter: twitter.com/FishdomOfficial
        </p>
        <p>
          わからないことがある場合は、テクニカルサポート: fishdom@playrix.com にメールでお問い合わせいただくか、ウェブのサポートポータルを参照してください: https://plrx.me/2IKJXyB6HR
        </p>
      </div>
    </div>
  </div>
...
<%
  try {
    ...
    // ステートメントの作成とSQL文の実行
    String sql = "SELECT star_rating, COUNT(*) as count FROM app_fishdom_review GROUP BY star_rating";
...
<%
  try {
    ...
    // ステートメントの作成とSQL文の実行
    String sql = "SELECT * FROM app_fishdom_review";
    Statement stmt = con.createStatement();
    ResultSet rs = stmt.executeQuery(sql);
