...
<body>
  ...
  <div style="display: flex; align-items: center;">
    ...
    <span style="font-size: 45px; font-weight: bold;">元祖チャリ走</span>
  </div>
  ...
  <div class="popup-overlay" id="popup-overlay">
    <div class="popup-content">
      ...
      <div id="collapse-content">
        ...
        <p>
          ■■チャリ走とは？■■■■
          迫りくる崖をジャンプして、ひたすら走り続けるだけの超シンプルゲーム！
          ２段ジャンプで広い崖、高い段差を超えていこう。
        </p>
          ■■遊び方■■■■
          画面をタップするだけでジャンプ！
          登場するライバル「佐藤」を押しのけてひたすら走りつづけよう！
          画面スライドでチャリを左右に操作し、ライバルの「佐藤」から逃げ切ろう。
        </p>
        <p>
          懐かしさはそのままに、グレードアップした「あの頃のチャリ走」をお楽しみください。
        </p>
      </div>
    </div>
  </div>
...
<%
  try {
    ...
    // ステートメントの作成とSQL文の実行
    String sql = "SELECT star_rating, COUNT(*) as count FROM app_charisog_review GROUP BY star_rating";
...
<%
  try {
    ...
    // ステートメントの作成とSQL文の実行
    String sql = "SELECT * FROM app_charisog_review";
    Statement stmt = con.createStatement();
    ResultSet rs = stmt.executeQuery(sql);
